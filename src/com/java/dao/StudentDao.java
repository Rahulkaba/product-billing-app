...
public static int save(StudentBean bean){
	...
	try{
		...
		PreparedStatement ps=con.prepareStatement("insert into manu.fee_student(name,email,sex,course,fee,paid,due,address,contact) values(?,?,?,?,?,?,?,?,?)");
...
public static int update(StudentBean bean){
	...
	try{
		Connection con=DB.getCon();
		PreparedStatement ps=con.prepareStatement("update manu.fee_student set name=?,email=?,sex=?,course=?,fee=?,paid=?,due=?,address=?,contact=? where rollno=?");
		ps.setString(1,bean.getName());
		ps.setString(2,bean.getEmail());
		ps.setString(3,bean.getSex());
		ps.setString(4,bean.getCourse());
...
public static int delete(int rollno){
	...
	try{
		Connection con=DB.getCon();
		PreparedStatement ps=con.prepareStatement("delete from manu.fee_student where rollno=?");
		ps.setInt(1,rollno);
		status=ps.executeUpdate();
		con.close();
	}catch(Exception ex){System.out.println(ex);}
...
public static int deleteByName(String name){
	...
	try{
		Connection con=DB.getCon();
		PreparedStatement ps=con.prepareStatement("delete from manu.fee_student where name=?");
		ps.setString(1,name);
		status=ps.executeUpdate();
		con.close();
	}catch(Exception ex){System.out.println(ex);}
...
public static List<StudentBean> getAllRecords(){
	...
	try{
		Connection con=DB.getCon();
		PreparedStatement ps=con.prepareStatement("select * from manu.fee_student");
...
public static List<StudentBean> getDues(){
	...
	try{
		Connection con=DB.getCon();
		PreparedStatement ps=con.prepareStatement("select * from manu.fee_student where due>0");
		ResultSet rs=ps.executeQuery();
		while(rs.next()){
			StudentBean bean=new StudentBean();
			bean.setName(rs.getString(1));
...
public static StudentBean getRecordByRollno(int rollno){
	...
	try{
		Connection con=DB.getCon();
		PreparedStatement ps=con.prepareStatement("select * from manu.fee_student where rollno=?");
		ps.setInt(1,rollno);
		ResultSet rs=ps.executeQuery();
		while(rs.next()){
			bean.setName(rs.getString(1));
